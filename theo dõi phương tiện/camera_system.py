...
class SmartLaneConfig:
    def __init__(self, frame_width: int, frame_height: int, auto_default=False):
        self.frame_width = frame_width
        self.frame_height = frame_height
        self.lanes = {}
        self.stop_lines = []

        # ✅ CHỈ tạo lane mặc định khi muốn
        if auto_default:
    ...
    def load_from_json(self, path: str = "lane_config.json"):
        if not Path(path).exists():
            raise FileNotFoundError(f"Không tồn tại file: {path}")

        with open(path, 'r', encoding='utf-8') as f:
            config = json.load(f)

        # ✅ XÓA SẠCH TRƯỚC KHI LOAD
        self.lanes = {}
        self.stop_lines = []
...
class OptimizedTrafficSystem:
    ...
    def __init__(self, video_path: str):
        ...
        if not ret:
            cap.release()
            raise ValueError(f"Không thể đọc frame từ video: {video_path}")
        ...
        print(f"✓ Video loaded: {w}x{h}, {self.fps}fps, {self.total_frames} frames")
        
        # Khởi tạo các module
        self.camera_analyzer = CameraAnalyzer()
        self.adaptive_detector = AdaptiveDetector(h)
        self.lane_config = SmartLaneConfig(w, h, auto_default=False)

        if Path("lane_config.json").exists():
            self.lane_config.load_from_json("lane_config.json")
        else:
            raise FileNotFoundError("❌ Không tìm thấy lane_config.json. Hãy calibrate trước.")
    ...
    def process_video(self, output_path: str = "output_optimized.mp4"):
        """Xử lý video chính - Hỗ trợ mọi format bao gồm ASF"""
        cap = cv2.VideoCapture(self.video_path)
        
        if not cap.isOpened():
            print(f"✗ Không thể mở video: {self.video_path}")
            print("💡 Thử:")
            print("   1. Cài ffmpeg: pip install ffmpeg-python")
            print("   2. Hoặc convert sang .mp4: ffmpeg -i video.asf video.mp4")
            return
        ...
        # Fix: Một số file ASF có FPS = 0
        if fps <= 0:
            fps = 25  # Default FPS
            print(f"⚠ FPS không xác định, sử dụng mặc định: {fps}")
        
        # Sử dụng codec H264 cho output (tương thích tốt hơn)
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(output_path, fourcc, fps, (w, h))
        
        if not out.isOpened():
            print("⚠ Không thể tạo video writer. Thử codec khác...")
        ...
        while True:
            ret, frame = cap.read()
            if not ret:
                # Một số file ASF có thể kết thúc sớm
                if frame_count == 0:
            ...
            if frame_count % 30 == 0:
                if total_frames > 0:
                    ...
                    print(f"Progress: {progress:.1f}% | Frame: {frame_count}/{total_frames} | "
                          f"FPS: {fps_current:.1f} | Tracks: {self.stats['total_tracks']}")
                else:
        ...
        print(f"✓ Tổng detections: {self.stats['total_detections']}")
        print(f"✓ Vi phạm phát hiện:")
...
def check_video_file(video_path: str) -> bool:
    ...
    Hỗ trợ: .mp4, .avi, .asf, .wmv, .mkv, .flv, .mov
    """
    supported_formats = ['.mp4', '.avi', '.asf', '.wmv', '.mkv', '.flv', '.mov']
    ...
    if ext not in supported_formats:
        print(f"⚠ Định dạng {ext} có thể không được hỗ trợ")
        print(f"   Định dạng được khuyến nghị: {', '.join(supported_formats)}")
    
    # Thử mở file
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        print(f"✗ Không thể mở video: {video_path}")
        print("   Thử cài đặt codec: sudo apt-get install ffmpeg")
        cap.release()
        return False
    ...
    print(f"✓ Video hợp lệ: {video_path}")
    print(f"  Format: {ext}")
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='detect',
                       ...
                       help='Chế độ chạy')
    parser.add_argument('--video', type=str, default='video.asf',
                       help='Đường dẫn video (hỗ trợ .asf, .mp4, .avi, ...)')
    parser.add_argument('--output', type=str, default='output_optimized.mp4',
                       help='Video output')
    
    args = parser.parse_args()
    
    # Kiểm tra file video trước
    print("\n" + "="*80)
    print("KIỂM TRA VIDEO INPUT")
    print("="*80)
    if not check_video_file(args.video):
        print("\n✗ Không thể tiếp tục. Vui lòng kiểm tra file video.")
        return
    print("="*80 + "\n")
